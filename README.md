...
- **mode** (*Optional*): Specifies which sensors to create. 'friendly' will only create one sensor per day of forecast, and will have all the forecast information as sensor attributes, 'sensors' will create separate sensors for each value returned by the BOM data. 'all' returns both the friendly sensors and each individual sensor as well. Default = 'friendly'.
