    ...
    def get_reading(self, condition, index):
        ...
        if condition in ('detailed_summary','fire_danger','uv_alert'):
            ...
            if PRODUCT_ID_LAT_LON_LOCATION[self._product_id][3] == 'City':
                ...
                detailed_summary = self._data.find(_FIND_QUERY.format(index, SENSOR_TYPES[condition][0])).text
        ...
        state = self._data.find(_FIND_QUERY.format(index, SENSOR_TYPES[condition][0]))
        if condition == 'icon':
